...
int factory_init(pfac pf,thread_func_t threadfunc,int thread_num,int capacity)
{
	...
	pf->pth_id=calloc(thread_num,sizeof(pthread_t));//存放线程id申请空间
...
int tcp_start_listen(int *psfd,char *ip,char *port,int backlog)
{
	...
	int reuse=1;
	int ret;
	ret=setsockopt(sfd,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(int));//端口号重用
	check_error(-1,ret,"setsockopt");
	struct sockaddr_in ser;
	bzero(&ser,sizeof(ser));
	ser.sin_family=AF_INET;
	ser.sin_port=htons(atoi(port));//端口号转换为网络字节序
